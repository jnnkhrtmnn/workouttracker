...
server <- function(input, output, session) {
  ...
  output$choice_tabl <- renderReactable({
    reactable(data.frame(exercises),
              ...
              selection="multiple",
              onClick="select",
              defaultSelected = c(1:min(5,length(exercises))), #match(c("bench_press", "squat", "press", "deadlift", "power_clean"),exercises),#,
